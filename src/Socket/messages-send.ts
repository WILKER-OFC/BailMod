...
export const makeMessagesSocket = (config: SocketConfig) => {
	...
	const sendReceipts = async (keys: WAMessageKey[], type: MessageReceiptType) => {
		// aggregateMessageKeysNotFromMe can return undefined entries depending on key validity
		const recps = aggregateMessageKeysNotFromMe(keys).filter(Boolean) as Array<{
			jid: string
			participant: string | undefined
			messageIds: string[]
		}>
		// Send in parallel (bounded) to minimize latency when many messages are read at once
		const concurrency = 5
		let i = 0
		const workers = Array.from({ length: Math.min(concurrency, recps.length) }, async () => {
			while (i < recps.length) {
				// with noUncheckedIndexedAccess, indexing returns T | undefined
				const recp = recps[i++]
				if(!recp) {
					continue
				}
				const { jid, participant, messageIds } = recp
	...
	const relayMessage = async (
		...
		if (
			...
			if (contentType === 'interactiveMessage' || contentType === 'buttonsMessage') {
				...
				// list message only supported in private chat
				bizNode.content = [
					{
						tag: 'list',
						attrs: {
