...
export const makeEventBuffer = (logger: ILogger): BaileysBufferableEventEmitter => {
	...
	// Debounced flush handle to avoid creating one timeout per buffered function
		...
		createBufferedFunction(work) {
			return async (...args) => {
				...
				try {
					...
					if (bufferCount === 0) {
						// Flush ASAP (next tick) to minimize latency for real-time bots,
						// while still allowing nested buffered work to batch.
						if (!flushPendingTimeout) {
