...
const spawnAndCollect = (cmd: string, args: string[], timeoutMs = 30_000): Promise<SpawnResult> =>
	new Promise((resolve, reject) => {
		...
		const t = setTimeout(() => {
			...
			reject(new Boom(`Command timed out: ${cmd}`, { data: { cmd, args } }))
		}, timeoutMs)
		...
		child.on('close', code => {
			...
			if (code === 0) {
				...
				reject(new Boom(`Command failed: ${cmd}`, { data: { cmd, args, code, stderr } }))
			}
		})
	})

/** Extracts video thumb using ffmpeg (safe args, handles spaces) */
...
/** Returns media duration in seconds using ffprobe (requires ffmpeg install) */
export const getVideoDuration = async (filePath: string, logger?: ILogger) => {
	try {
		const meta = await getVideoMetadata(filePath, logger)
		return meta?.duration
	} catch (e) {
		logger?.debug(`Failed to get video duration via ffprobe: ${e}`)
		return undefined
	}
}

/** Returns video duration + dimensions via ffprobe (requires ffmpeg install) */
export const getVideoMetadata = async (filePath: string, logger?: ILogger) => {
	try {
		const { stdout } = await spawnAndCollect('ffprobe', [
			'-v',
			'error',
			'-select_streams',
			'v:0',
		...
		logger?.debug(`Failed to get video metadata via ffprobe: ${e}`)
		return undefined
	}
}

/** Returns audio format + codec info via ffprobe (requires ffmpeg install) */
export const getAudioMetadata = async (filePath: string, logger?: ILogger) => {
	try {
		const { stdout } = await spawnAndCollect('ffprobe', [
			'-v',
			'error',
		...
		logger?.debug(`Failed to get audio metadata via ffprobe: ${e}`)
		return undefined
	}
}

/** Converts any audio into OGG/Opus (WhatsApp voice note compatible) */
export const convertAudioToOggOpus = async (inputPath: string, outputPath: string, logger?: ILogger) => {
	try {
		await spawnAndCollect('ffmpeg', [
			'-hide_banner',
			'-nostdin',
		...
		logger?.warn({ err: e }, 'Failed to convert audio to OGG/Opus with ffmpeg')
		throw e
	}
}
...
export async function generateThumbnail(
	...
	if (mediaType === 'image') {
		...
		try {
			const timesToTry = Array.from(
				new Set([videoTime, '00:00:01.00', '00:00:00.50', '00:00:00.10', '00:00:00.00'])
			)
			let buff: Buffer | undefined
			for (const t of timesToTry) {
				try {
					await extractVideoThumb(file, imgFilename, t, { width: thumbWidth, height: thumbWidth })
					buff = await fs.readFile(imgFilename)
					break
				} catch (e) {
					options.logger?.debug(`could not extract video thumb at ${t}: ${e}`)
				}
			}

			if (buff) {
				// Ensure thumbnail is small & WhatsApp-friendly (JPEG, low quality)
				try {
					const processed = await extractImageThumb(buff, thumbWidth)
					thumbnail = processed.buffer
				} catch {
					thumbnail = buff
				}
			}
...
export const encryptedStream = async (
	media: WAMediaUpload,
	mediaType: MediaType,
	{ logger, saveOriginalFileIfRequired, opts, mediaKey: mediaKeyOverride }: EncryptedStreamOptions = {}
) => {
	const { stream, type } = await getStream(media, opts)

	logger?.debug('fetched media stream')

	const mediaKey = mediaKeyOverride || Crypto.randomBytes(32)
