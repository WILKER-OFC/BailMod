	...
	const handleMessage = async (node: BinaryNode) => {
		...
		try {
			...
			if (!isNewsletter) {
				...
				// Fire-and-forget receipts to avoid blocking event flushing.
				// This improves bot response latency without changing semantics.
				void sendReceipt(msg.key.remoteJid!, participant!, [msg.key.id!], type).catch(err => {
					logger.warn({ err, key: msg.key }, 'failed sending delivery receipt')
				})
				...
				if (isAnyHistoryMsg) {
					...
					void sendReceipt(jid, undefined, [msg.key.id!], 'hist_sync').catch(err => {
						logger.warn({ err, key: msg.key }, 'failed sending history sync receipt')
					})
				}
			} else {
				await sendMessageAck(node)
